...
public extension MBServiceCenter {
    ...
    @objc class var OCProtocolMatchingPatternStr : String {
        set {
            if (try? NSRegularExpression(pattern: newValue)) != nil {
                ...
                MBModuleLogger.errorLog("error: invalid oc matching pattern \(newValue) trying to be set", extra: nil)
            }
        }
        get {
    ...
    func registerService<Service>(of serviceType: Service.Type, withImplClass implClass: AnyClass){
        registerService(of: MBServiceCenter.serviceProtocol(of: serviceType)!, withImplClass: implClass)
    }
    
    // 发现
    func service<Service>(of serviceType: Service.Type, fromContext context:MBContextProtocol?) ->Service?{
        let serviceProtocol = MBServiceCenter.serviceProtocol(of: serviceType)!
