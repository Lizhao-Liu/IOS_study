...
//  MBFoundation
//
//  Created by rensihao on 2021/2/20.
//
...
public struct DataCompressor {
    ...
    static func compress(_ data: Data, algorithm: DataCompressor.Algorithm) -> Data? {
        if Data.mb_nilBaseAddress(data) { return nil }
        return data.withUnsafeBytes { (ptr) -> Data? in
            let sourcePtr: UnsafePointer<UInt8> = ptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            let config = (operation: COMPRESSION_STREAM_ENCODE, algorithm: algorithm.lowLevelType)
    ...
    static func decompress(_ data: Data, algorithm: DataCompressor.Algorithm) -> Data? {
        if Data.mb_nilBaseAddress(data) { return nil }
        return data.withUnsafeBytes { (ptr) -> Data? in
            let sourcePtr: UnsafePointer<UInt8> = ptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            let config = (operation: COMPRESSION_STREAM_DECODE, algorithm: algorithm.lowLevelType)
            return perform(config, source: sourcePtr, sourceSize: data.count)
        }
    }
    ...
    /// - note: Fixed at compression level 5 (best trade off between speed and time)
    static func zip(_ data: Data) -> Data? {
        let header = Data([0x78, 0x5e])

        if Data.mb_nilBaseAddress(data) { return nil }
        let deflated = data.withUnsafeBytes { (ptr) -> Data? in
            let sourcePtr: UnsafePointer<UInt8> = ptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            let config = (operation: COMPRESSION_STREAM_ENCODE, algorithm: COMPRESSION_ZLIB)
        ...
        var adler = self.adler32(data).checksum.bigEndian
    ...
    /// - returns: uncompressed data
    static func unzip(_ data: Data, skipCheckSumValidation: Bool = true) -> Data? {
        // 2 byte header + 4 byte adler32 checksum
        let overhead = 6
        guard data.count > overhead else { return nil }

        if Data.mb_nilBaseAddress(data) { return nil }
        let header: UInt16 = data.withUnsafeBytes { (ptr) -> UInt16 in
            let sourcePtr: UnsafePointer<UInt16> = ptr.baseAddress!.assumingMemoryBound(to: UInt16.self)
            return sourcePtr.pointee.bigEndian
        }
        ...
        let cresult: Data? = data.withUnsafeBytes { (ptr) -> Data? in
            let sourcePtr: UnsafePointer<UInt8> = ptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            let source = sourcePtr.advanced(by: 2)
            let config = (operation: COMPRESSION_STREAM_DECODE, algorithm: COMPRESSION_ZLIB)
            return perform(config, source: source, sourceSize: data.count - overhead)
        }
        ...
        let cksum: UInt32 = data.withUnsafeBytes { (ptr) -> UInt32 in
            let bytePtr: UnsafePointer<UInt8> = ptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
        ...
        return cksum == self.adler32(inflated).checksum ? inflated : nil
    }
    ...
    static func gzip(_ data: Data) -> Data? {
        ...
        var crc32: UInt32 = self.crc32(data).checksum.littleEndian
    ...
    static func gunzip(_ data: Data) -> Data? {
        ...
        if Data.mb_nilBaseAddress(data) { return nil }
        let hdr: GZipHeader = data.withUnsafeBytes { (metaptr) -> GZipHeader in
            let ptr: UnsafePointer<UInt8> = metaptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            // +---+---+---+---+---+---+---+---+---+---+
        ...
        let ftr: GZipFooter = data.withUnsafeBytes { (ptr) -> GZipFooter in
            let bptr: UnsafePointer<UInt8> = ptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            // +---+---+---+---+---+---+---+---+
            // |     CRC32     |     ISIZE     |
            // +---+---+---+---+---+---+---+---+
            return bptr.advanced(by: data.count - 8).withMemoryRebound(to: UInt32.self, capacity: 2) { ptr in
        ...
        let cresult: Data? = data.withUnsafeBytes { (metaptr) -> Data? in
            let ptr: UnsafePointer<UInt8> = metaptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
            var pos = 10 ; let limit = data.count - 8
...
public struct Crc32: CustomStringConvertible {
    ...
    public mutating func advance(withChunk chunk: Data) {
        if let fastCrc32 = Crc32.zLibCrc32,
           !Data.mb_nilBaseAddress(chunk) {
            checksum = chunk.withUnsafeBytes({ (metaptr) -> UInt32 in
                let ptr: UnsafePointer<UInt8> = metaptr.baseAddress!.assumingMemoryBound(to: UInt8.self)
...
public extension Data {
    static func mb_nilBaseAddress(_ data: Data?) -> Bool {
        guard let data = data else { return true }
        
        var failed = false
        data.withUnsafeBytes({ ptr in
            if (ptr.baseAddress == nil) {
                failed = true
            }
        })
        return failed
