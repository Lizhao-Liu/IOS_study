...
const contributors = overview.contributors
...
const midendDevelopers = [
  contributors['kang.ji'],
  contributors['yingkun.li'],
  contributors['zhiguo.cheng'],
  contributors['qicheng.zhang'],
  contributors['jian.sun1'],
]
/** 设计团队介绍 */
  ...
  return (
    <>
      ...
      <main>
        ...
        <Text>DLC秉承设计·友爱·创新的价值观点，致力为满帮集团用户体验进行赋能。</Text>
        <div>
          {designers.map((persons, index) => (
            <div style={{ display: 'flex', marginTop: 48 }} key={index}>
              {persons.map((person, i) => (
                <div key={i} className='__design-about'>
                  <Avatar
                    shape='square'
                    src={person.avatar + '?x-oss-process=image/format,webp/interlace,1'}
        ...
        <div style={{ marginTop: 60, display: 'flex' }}>
          {midendDevelopers.map((person, i) => (
            <div key={i} className='__design-dev'>
              <Avatar
                src={person.avatar.replace(/\?.*/, '?x-oss-process=image/format,webp/interlace,1')}
                className='__design-dev-avatar'
                size={100}
              />
              <div className='__design-dev-name'>{person.name}</div>
              <div className='__design-dev-title'>前端开发工程师</div>
            </div>
          ))}
        </div>
      </main>
...
function SubTitle(props: PropsWithStringChildren) {
  return <p style={createStyle(24, 500, '#141414', 33, 40)}>{props.children}</p>
}

function Text(props: PropsWithStringChildren) {
  return <p style={createStyle(14, 400, '#4c4c4c', 22, 16)}>{props.children}</p>
}
